	...
	void interact(std::string const & file)
	{
		...
		while(choice!=5)
		{
			...
			else
			{
				switch (choice)
				{
					case 2: std::cout << db_ops::insert(con,conf["def_table"]) <<" rows inserted.\n---------\n"; break;
					case 3: std::cout << db_ops::update(con,conf["def_table"]) << " rows updated.\n---------\n"; break;
					case 4: std::cout << db_ops::del(con,conf["def_table"]) << " rows deleted.\n--------\n"; break;
				}
			}
